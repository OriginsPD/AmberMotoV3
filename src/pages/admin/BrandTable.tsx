...
const BrandTable = () => {
	const { indexBrand, brands, isLoaded } = BikeBrandApi();
	useEffect(() => {
		indexBrand();
	}, []);

	const [pageCount, setPageCount] = useState(0);
	const [currentBrand, setCurrentBrand] = useState<BrandProp[]>(
		{} as BrandProp[]
	);

	useEffect(() => {
		setPageCount(Math.ceil(Object.keys(brands).length / 5 - 1));
		setCurrentBrand(Object.values(brands).slice(5, 10));
	}, [brands]);
	...
	const handleClick = async (data: { selected: any }) => {
		...
		setCurrentBrand(Object.values(brands).slice(firstPageIndex, lastPageIndex));
				...
				<div className="sm:flex sm:items-center">
					<div className="sm:flex-auto">
						...
						<p className="mt-2 text-sm capitalize text-gray-700">
							A list of all the Brands That bikes may fall within.
						</p>
					</div>
