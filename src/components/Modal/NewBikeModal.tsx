...
const schema = yup.object({
	...
	fileUpload: yup
		.mixed()
		.test("required", "Please Upload Motor Bike Image", (value) => {
			return value && value.length;
		})
		.test("type", "We only support jpeg/png", function (value) {
			return (
				value && value[0] && value[0].type.includes("image/jpeg", "image/png")
			);
		}),
});

const NewBikeModal = ({ isOpen, setIsOpen, toggleModal }: NewBikeProps) => {
	const { state, storeInfo, resetInfo, storeImage } = useForms();
	...
	const { BikeCreate, bikeDetailIndex } = BikeDetailApi();
	...
	return (
		<div>
			<Transition appear show={isOpen} as={Fragment} afterLeave={clearForm}>
																...
																<div className="col-span-full flex space-x-2">
																	{preview ? (
																		<div className="h-40 w-1/2 ">
																			...
																			<img
																				className="inline-block h-40 w-40  object-fill ring-2 ring-white"
																				src={preview}
																				alt=""
																			/>
																		</div>
															...
															<div className="ml-3">
																...
																<div className="mt-2 text-sm text-red-700">
																	...
																	>
																		...
																		{errors.fileUpload && (
																			<li> {errors.fileUpload?.message} </li>
																		)}
																	</ul>
																</div>
															</div>
